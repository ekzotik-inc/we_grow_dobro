...
async def main() -> None:
    ...
    print("\nСТАТУС УЧАСТНИКА")
    sent.clear()
    await act(dp, bot, ADMIN, f"adm:dq:{uid}", f"adm:dq_r:{uid}:idle")
    check("дисквалификация за неактивность", USER, "неактивность")
    sent.clear()
    await act(dp, bot, ADMIN, f"adm:reinstate:{uid}")
    sent.clear()
    await act(dp, bot, ADMIN, f"adm:dq:{uid}", f"adm:dq_own:{uid}", ">нарушение правил")
    check("дисквалификация своей причиной", USER, "правил")
    sent.clear()
    await act(dp, bot, ADMIN, f"adm:reinstate:{uid}")
